...
class TimeEventControllerTest {
    ...
    void whenAllEventsForOnePersonAreRequestedA200WithOnlyTheseEventsAreReturned() {
        ...
        doReturn(timeEventsInRepository).when(timeEventService).getLastActiveTimeEvents(userId, false);
        ResponseEntity<Iterable> response = timeEventController.findTimeEvents(userId, false);

        Iterable optionalIterable = response.getBody();
        assertEquals(timeEventsInRepository, optionalIterable);
        assertEquals(HttpStatus.OK, response.getStatusCode());
        verify(timeEventService).getLastActiveTimeEvents(userId, false);
    }
    ...
    void lastActiveTimeEventIsReturned() {
        ...
        doReturn(lastTimeEvent).when(timeEventService).getLastActiveTimeEvents(id, true);
        ResponseEntity<Iterable> lastActiveTimeEventResponse = timeEventController.findTimeEvents(id, true);
